        ...
        private void PatchConnectionEvents(ref SystemState state)
        {
            ...
            for (var i = 0; i < connectionEvents.Length; i++)
            {
                ...
                if (!connections.IsCreated)
                {
                    ...
                    connections = m_ConnectionQuery.ToComponentDataArray<NetworkStreamConnection>(Allocator.TempJob);
                    entities = m_ConnectionQuery.ToEntityArray(Allocator.TempJob);
                }
            ...
            {
                var incorrectlyDisposedConnectionEntities = m_IncorrectlyDisposedConnectionsQuery.ToEntityArray(Allocator.TempJob);
                var incorrectlyDisposedConnections = m_IncorrectlyDisposedConnectionsQuery.ToComponentDataArray<NetworkStreamConnection>(Allocator.TempJob);
