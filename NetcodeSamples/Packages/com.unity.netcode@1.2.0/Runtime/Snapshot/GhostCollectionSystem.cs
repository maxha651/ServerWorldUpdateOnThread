...
namespace Unity.NetCode
{
    ...
    public partial struct GhostCollectionSystem : ISystem
    {
        ...
        {
            using var entityQueryBuilder = new EntityQueryBuilder(Allocator.TempJob).WithAll<GhostPrefabMetaData, Prefab, GhostPrefabRuntimeStrip>();
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            {
                using var ghostEntities = m_RegisterGhostTypesQuery.ToEntityArray(Allocator.TempJob);
                using var ghostTypes = m_RegisterGhostTypesQuery.ToComponentDataArray<GhostType>(Allocator.TempJob);
                var ghostCollectionList = ghostCollectionFromEntity[collectionSingleton];
            ...
            if (ghostPrefabSerializerErrors > 0)
            {
                ...
                var connections = m_AllConnectionsQuery.ToEntityArray(Allocator.TempJob);
        ...
        private void RuntimeStripPrefabs(ref SystemState state, in NetDebug netDebug)
        {
            ...
            using var prefabEntities = m_RuntimeStripQuery.ToEntityArray(Allocator.TempJob);
            using var metaDatas = m_RuntimeStripQuery.ToComponentDataArray<GhostPrefabMetaData>(Allocator.TempJob);
            for (int i = 0; i < prefabEntities.Length; i++)
            {
                ...
                if (removeOnWorld.Length > 0)
                {
                    ...
                    var entities = state.EntityManager.GetBuffer<LinkedEntityGroup>(prefabEntity).ToNativeArray(Allocator.TempJob);
