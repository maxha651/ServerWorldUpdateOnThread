...
namespace Unity.NetCode
{
    ...
    public static class GhostPrefabCreation
    {
            ...
            NativeArray<int> sendMasksOverride, NativeArray<ulong> variants)
        {
            ...
            var serverComponents = new NativeList<ulong>(allComponents.Length, Allocator.TempJob);
            var serverVariants = new NativeList<ulong>(allComponents.Length, Allocator.TempJob);
            var serverSendMasks = new NativeList<int>(allComponents.Length, Allocator.TempJob);
            var removeOnServer = new NativeList<GhostPrefabBlobMetaData.ComponentReference>(allComponents.Length, Allocator.TempJob);
            var removeOnClient = new NativeList<GhostPrefabBlobMetaData.ComponentReference>(allComponents.Length, Allocator.TempJob);
            var disableOnPredicted = new NativeList<GhostPrefabBlobMetaData.ComponentReference>(allComponents.Length, Allocator.TempJob);
            var disableOnInterpolated = new NativeList<GhostPrefabBlobMetaData.ComponentReference>(allComponents.Length, Allocator.TempJob);
        ...
        public static void FinalizePrefabComponents(Config ghostConfig, EntityManager entityManager,
            ...
            NetcodeConversionTarget target, NativeArray<GhostPrefabType> prefabTypes)
        {
            var entities = new NativeArray<Entity>(allComponents.Length, Allocator.TempJob);
            int compIdx = 0;
            for (int k = 0; k < linkedEntities.Length; ++k)
            {
                var numComponents = componentCounts[k];
            ...
            var removedFromClient = new NativeArray<bool>(allComponents.Length, Allocator.TempJob);
        ...
        private static NativeArray<ComponentType> GetNotBakingComponentTypes(EntityManager entityManager, Entity entity, ComponentType linkedEntityGroupComponentType)
        {
            ...
            NativeList<ComponentType> relevantComponents = new NativeList<ComponentType>(components.Length, Allocator.TempJob);
        ...
        public static void CollectAllComponents(EntityManager entityManager, NativeArray<Entity> linkedEntities, out NativeList<ComponentType> allComponents, out NativeArray<int> componentCounts)
        {
            ...
            allComponents = new NativeList<ComponentType>(rootComponents.Length*linkedEntities.Length, Allocator.TempJob);
            componentCounts = new NativeArray<int>(linkedEntities.Length, Allocator.TempJob);
            ...
            NativeParallelHashMap<Component, ComponentOverride> overrides = default)
        {
            ...
            if (!overrides.IsCreated)
                overrides = new NativeParallelHashMap<Component, ComponentOverride>(1, Allocator.TempJob);
            ...
            var linkedEntitiesArray = new NativeArray<Entity>(linkedEntityBuffer.Length, Allocator.TempJob);
            ...
            {
                allComponents = new NativeList<ComponentType>(256, Allocator.TempJob);
                componentCounts = new NativeArray<int>(linkedEntitiesArray.Length, Allocator.TempJob);
            ...
            var prefabTypes = new NativeArray<GhostPrefabType>(allComponents.Length, Allocator.TempJob);
            var sendMasksOverride = new NativeArray<int>(allComponents.Length, Allocator.TempJob);
            var variants = new NativeArray<ulong>(allComponents.Length, Allocator.TempJob);

            // TODO - Consider changing the API to pass this as an arg.
            using var collectionDataQuery = entityManager.CreateEntityQuery(new EntityQueryBuilder(Allocator.TempJob).WithAll<GhostComponentSerializerCollectionData>());
            ...
            using var codePrefabQuery = entityManager.CreateEntityQuery(new EntityQueryBuilder(Allocator.TempJob).WithAll<CodeGhostPrefab>());
