...
namespace Unity.NetCode
{
    ...
    partial struct PrespawnGhostInitializationSystem : ISystem, ISystemStartStop
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            var subSceneWithPrespawnGhosts = m_UninitializedScenes.ToComponentDataArray<SubSceneWithPrespawnGhosts>(Allocator.TempJob);
            var subScenesSections = m_UninitializedScenes.ToEntityArray(Allocator.TempJob);
            var readySections = new NativeList<int>(subScenesSections.Length, Allocator.TempJob);
            ...
            for (int i = 0; i < subScenesSections.Length; ++i)
            {
                ...
                var ghostTypes = m_Prespawns.ToComponentDataArray<GhostType>(Allocator.TempJob);
            ...
            var jobs = new NativeList<JobHandle>(readySections.Length, Allocator.TempJob);
            for (int readyScene = 0; readyScene < readySections.Length; ++readyScene)
            {
                var sceneIndex = readySections[readyScene];
                var sharedFilter = new SubSceneGhostComponentHash {Value = subSceneWithPrespawnGhosts[sceneIndex].SubSceneHash};
