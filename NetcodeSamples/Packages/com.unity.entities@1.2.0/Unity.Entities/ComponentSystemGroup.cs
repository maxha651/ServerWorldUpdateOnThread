...
namespace Unity.Entities
{
    ...
    public abstract unsafe partial class ComponentSystemGroup : SystemBase
    {
        ...
        public NativeList<SystemHandle> GetUnmanagedSystems(Allocator allocator = Allocator.TempJob)
        {
        ...
        /// <param name="allocator">Which allocator to use to allocate the returned list.</param>
        /// <returns>A NativeList of systems</returns>
        public NativeList<SystemHandle> GetAllSystems(Allocator allocator = Allocator.TempJob)
        {
        ...
        private void RemoveSystemsFromUnsortedUpdateList()
        {
            ...
            var newListIndices = new NativeArray<int>(largestID + 1, Allocator.TempJob);
        ...
        private void GenerateMasterUpdateList()
        {
            ...
            for (int i = 0; i < m_managedSystemsToUpdate.Count; ++i)
            {
                ...
                allElems[i] = new ComponentSystemSorter.SystemElement
                {
                    ...
                    updateBefore = new NativeList<int>(16, Allocator.TempJob),
                    nAfter = 0,
                };
            ...
            var lookupDictionary = new NativeHashMap<int, int>(16, Allocator.TempJob);

            var nativeHashMap =
                (NativeHashMap<SystemTypeIndex, int>*)UnsafeUtility.AddressOf(ref lookupDictionary);

            var badTypeIndices = new NativeHashSet<SystemTypeIndex>(16, Allocator.TempJob);
            ...
            {
                var enumerator = badTypeIndices.ToNativeArray(Allocator.TempJob);
            ...
            var elemBuckets = new []
            {
                new UnsafeList<ComponentSystemSorter.SystemElement>(systemsPerBucket[0], Allocator.TempJob),
                new UnsafeList<ComponentSystemSorter.SystemElement>(systemsPerBucket[1], Allocator.TempJob),
                new UnsafeList<ComponentSystemSorter.SystemElement>(systemsPerBucket[2], Allocator.TempJob),
            };
