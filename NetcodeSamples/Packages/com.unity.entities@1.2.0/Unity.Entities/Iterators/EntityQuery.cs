...
namespace Unity.Entities
{
    ...
    public class EntityQueryDesc : IEquatable<EntityQueryDesc>
    {
        ...
        public void Validate()
        {
            ...
            var allComponentTypeIds = new NativeArray<TypeIndex>(itemCount, Allocator.TempJob);
    ...
    internal unsafe partial struct EntityQueryImpl
    {
        ...
        {
            using var types = new NativeList<ComponentType>(128, Allocator.TempJob);
            ...
            using var typesArray = types.ToArray(Allocator.TempJob);
...
First chunk: entityCount={matchingChunkCache.ChunkIndices[0].Count}, archetype={_Access->EntityComponentStore->GetArchetype(matchingChunkCache.ChunkIndices[0])->ToString()}.");
    ...
    public unsafe struct EntityQuery : IDisposable, IEquatable<EntityQuery>
    {
        ...
        {
            var requiredComponentsInCurrentQuery = new NativeHashMap<int, ComponentType.AccessMode>(8, Allocator.TempJob);
        ...
        public bool CompareComponents(ComponentType[] componentTypes)
        {
            ...
            {
                var builder = new EntityQueryBuilder(Allocator.TempJob, types, componentTypes.Length);
        ...
        {
            var builder = new EntityQueryBuilder(Allocator.TempJob, (ComponentType*)componentTypes.GetUnsafeReadOnlyPtr(), componentTypes.Length);
            builder.FinalizeQueryInternal();
            return _GetImpl()->CompareQuery(builder);
        }
