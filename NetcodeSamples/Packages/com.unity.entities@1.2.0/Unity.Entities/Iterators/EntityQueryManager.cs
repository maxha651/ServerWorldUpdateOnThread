...
namespace Unity.Entities
{
    ...
    internal unsafe struct EntityQueryManager
    {
        ...
        ArchetypeQuery* CreateArchetypeQueries(ref UnsafeScratchAllocator unsafeScratchAllocator, in EntityQueryBuilder queryBuilder)
        {
            ...
            var allTypes = new UnsafeList<ComponentType>(32, Allocator.TempJob);
            var anyTypes = new UnsafeList<ComponentType>(32, Allocator.TempJob);
            var noneTypes = new UnsafeList<ComponentType>(32, Allocator.TempJob);
            var disabledTypes = new UnsafeList<ComponentType>(32, Allocator.TempJob);
            var absentTypes = new UnsafeList<ComponentType>(32, Allocator.TempJob);
            var presentTypes = new UnsafeList<ComponentType>(32, Allocator.TempJob);
            ...
            for (int q = 0; q != queryData.Length; q++)
            {
                ...
                if (isFilterWriteGroup)
                {
                    ...
                    var explicitList = new UnsafeList<ComponentType>(allTypes.Length + anyTypes.Length +
                                                                     ...
                                                                     absentTypes.Length + presentTypes.Length + 16, Allocator.TempJob);
            ...
            int requiredComponentCount)
        {
            ...
            if (queryData == null)
            {
                ...
                var allEnableableTypeIndices = new NativeList<TypeIndex>(totalComponentCount, Allocator.TempJob);
                for (int iAQ = 0; iAQ < archetypeQueryCount; ++iAQ)
                {
