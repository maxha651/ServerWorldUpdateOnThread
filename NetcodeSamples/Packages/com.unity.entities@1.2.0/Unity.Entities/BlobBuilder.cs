...
namespace Unity.Entities
{
    ...
    unsafe public partial struct BlobBuilder : IDisposable
    {
        ...
        public BlobAssetReference<T> CreateBlobAssetReference<T>(AllocatorManager.AllocatorHandle allocator) where T : unmanaged
        {
            ...
            var offsets = new NativeArray<int>(m_allocations.Length + 1, Allocator.TempJob);
            var sortedAllocs = new NativeArray<SortedIndex>(m_allocations.Length, Allocator.TempJob);
            ...
            sortedAllocs.Sort();
            var sortedPatches = new NativeArray<SortedIndex>(m_patches.Length, Allocator.TempJob);
